# 📘 A'dan Z'ye AudioCodes SBC Akademi

![AudioCodes Banner](https://img.shields.io/badge/AudioCodes-v7.20-blue?style=for-the-badge&logo=audiocodes)
![Status](https://img.shields.io/badge/Status-Complete-green?style=for-the-badge)
![Type](https://img.shields.io/badge/Type-Educational%20Book-orange?style=for-the-badge)

Ses teknolojileri dünyasına hoş geldiniz! Bu depo, bir yeni mezun mühendisin sıfırdan başlayıp **AudioCodes Mediant 800 SBC** (v7.20) uzmanı olmasına yardımcı olacak şekilde tasarlanmış, yaşayan bir eğitim kitabıdır.

---

## 📖 Kitap Hakkında

Bu proje, sadece teknik bir manuel değil, aynı zamanda sektörel tecrübelerle harmanlanmış bir rehberdir. İçerik, AudioCodes Türkiye Partneri olan **Nolto Teknoloji Anonim Şirketi**'nin saha tecrübelerinden ilham alan örneklerle zenginleştirilmiştir.

### 🎯 Neler Öğreneceksiniz?
*   SBC mimarisi ve neden kritik olduğu.
*   Microsoft Teams Direct Routing kurulumu.
*   SIP ve RTP paketlerinin derinlemesine analizi.
*   Saha kurulumlarında hayat kurtaran "Hardening" ve "Troubleshooting" teknikleri.

---

## 🗺️ Eğitim Müfredatı

Kitabı sırasıyla takip etmeniz önerilir:

### 🟢 1. Temeller ve Sektörel Giriş
*   [SBC Nedir ve Neden Gereklidir?](docs/00-sektorel-bakis/01-sbc-nedir-ve-sektorel-konumu.md)
*   [Neden AudioCodes Seçmeliyiz?](docs/00-sektorel-bakis/02-neden-audiocodes.md)
*   [Teams Direct Routing Dünyası](docs/00-sektorel-bakis/03-teams-direct-routing.md)

### 🔵 2. Ürün Ailesi ve Ekosistem
*   [MediaPack vs Mediant](docs/01-audiocodes-urun-ailesi/01-mediapack-vs-mediant.md)
*   [Kapasite ve Donanım Rehberi](docs/01-audiocodes-urun-ailesi/02-mediant-donanim-ve-kapasite.md)
*   [OVOC ve CHAMPS Operasyonları](docs/02-ekosistem-ve-operasyon/01-ovoc-nedir-nasil-calisir.md)

### 🟡 3. Teknik Yapılandırma ve Uygulama
*   [IP Arayüzleri ve VLAN](docs/03-cekirdek-yapilandirma/01-ip-interfaces-ve-vlan.md)
*   [Yönlendirme (Routing) Mantığı](docs/04-sinyallesme-ve-yonlendirme/03-ip-to-ip-routing.md)
*   [High Availability (HA) Yedeklilik](docs/04-sinyallesme-ve-yonlendirme/04-high-availability-ha.md)
*   [Medya ve DSP Kaynak Yönetimi](docs/05-guvenlik-ve-medya/06-medya-kaynak-yonetimi-ve-dsp.md)
*   [Ses Kalitesi (MOS) İzleme](docs/05-guvenlik-ve-medya/07-ses-kalitesi-izleme-rtcp-xr.md)
*   [Güvenlik (SBC Hardening)](docs/05-guvenlik-ve-medya/04-hardening-ve-dos-korumasi.md)

### 🔴 4. Referanslar ve Ekler
*   [SIP Terimler Sözlüğü](docs/08-ekler-ve-referanslar/01-sip-terimler-sozlugu.md)
*   [Regex Kütüphanesi](docs/08-ekler-ve-referanslar/02-regex-kutuphanesi.md)
*   [Hata Kodları ve Sorun Giderme](docs/07-bakim-ve-sorun-giderme/01-syslog-ve-message-log-okuma.md)

---

## 📈 Öğrenme Yol Haritası
Kendi ilerlemenizi takip etmek için [ROADMAP.md](ROADMAP.md) dosyasını kullanabilirsiniz.

## 🛡️ Telif Hakkı ve Kullanım
Bu içerik **mrzcn** tarafından hazırlanmıştır ve tüm hakları saklıdır. İçerik dijital filigranlar ile korunmaktadır. Detaylar için [LICENSE](LICENSE) dosyasını inceleyiniz.

---
*Bu proje Nolto Teknoloji A.Ş.'den bağımsız bir eğitim girişimidir.*
